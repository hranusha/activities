<x-admin.wrapper>
    ...
    <x-admin.add-link href="{{ route('admin.activity.create') }}">
        {{ __('Add Activity') }}
    </x-admin.add-link>
            ...
            <x-admin.grid.table>
                <x-slot name="head">
                    <tr class="bg-base-200">
                        ...
                        <x-admin.grid.th>
                            @include('admin.includes.sort-link', ['label' => 'Date', 'attribute' => 'date'])
                        </x-admin.grid.th>
                        <x-admin.grid.th>
                            {{ __('Actions') }}
                        </x-admin.grid.th>
                ...
                @foreach($activities as $activity)
                    <tr>
                        <x-admin.grid.td>
                            <div>
                                <a href="{{route('admin.activity.show', $activity->id)}}" class="no-underline hover:underline text-cyan-600">{{ $activity->title }}</a>
                            </div>
                        </x-admin.grid.td>
                        <x-admin.grid.td>
                            <div>
                        ...
                        <x-admin.grid.td>
                            <form action="{{ route('admin.activity.destroy', $activity->id) }}" method="POST">
                                <div>
                                    <a href="{{route('admin.activity.edit', $activity->id)}}" class="btn btn-square btn-ghost">
